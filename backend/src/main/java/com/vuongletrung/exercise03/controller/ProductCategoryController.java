...
import com.vuongletrung.exercise03.service.ProductCategoryService;
...
public class ProductCategoryController {
    ...
    // Xóa tất cả ProductCategory có chứa categoryId nằm trong khu vực có chứa
    // productId
    @DeleteMapping("/product/{productId}/category/{categoryId}")
    public ResponseEntity<String> deleteProductCategoriesByProductIdAndCategoryId(
            @PathVariable("productId") String productId,
            @PathVariable("categoryId") String categoryId) {
        productCategoryService.deleteProductCategoriesByProductIdAndCategoryId(productId, categoryId);
        return new ResponseEntity<>(
                "All ProductCategories with productId " + productId + " and categoryId " + categoryId
