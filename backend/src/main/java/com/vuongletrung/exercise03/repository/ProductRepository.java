...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query("SELECT p FROM Product p JOIN p.productTags pt WHERE pt.tag.tagId = :tagId ORDER BY p.createdAt DESC, p.updatedAt DESC")
    List<Product> findProductsByTagIdOrderByCreatedAtAndUpdatedAtDesc(@Param("tagId") String tagId);
